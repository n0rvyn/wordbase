...
- **`/en/*` is this site's own machine rendition, not an App Store localization.** `localizePage`/`localizePost` (`packages/web/src/lib/localize.ts`) pull per-block renditions from the i18n translation-memory cache and fall back to the source: `enContent ?? page.content`, so an `/en/<slug>` page with no rendition renders the **Chinese body under an English shell**. Only blocks a human reviewed via `i18n_put_cache` are real English. Apps are the exception that causes the confusion — `localizeApp` overlays `meta.i18n.en`, which is the author's own English copy from the App Store listing, so on `/en/apps/*` the English *is* store copy. ⛔ **硬规定：提交给 App Store Connect 的 Support / Privacy URL 一律是不带 `/en` 的 `https://norvyn.com/<slug>`，任何 locale 都不提交 `/en/` URL。** 其它项目通过远端 MCP 建页时看到的是 `page_create` / `page_publish` 的 tool description（该规则已写在那里，改动随 API 部署才对外生效）。
  - 来源：session_01HzEJeHCtL6aT7Eor2QpNHq 第 2 轮用户原话「写进去，硬规定，ASC 一律提交不带 /en 的 URL」
