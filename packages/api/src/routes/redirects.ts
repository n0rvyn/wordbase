...
redirectsRouter.get('/', authMiddleware, requireScope('redirects:read'), async (c) => {
  const redirects = await redirectService.listRedirects();
  return c.json(redirects);
});

redirectsRouter.post('/', authMiddleware, requireScope('redirects:write'), async (c) => {
...
redirectsRouter.delete('/:id', authMiddleware, requireScope('redirects:write'), async (c) => {
