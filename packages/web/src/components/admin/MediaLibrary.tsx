  ...
  return (
    <div>
      <div class="flex items-center justify-between mb-6">
        <div>
          <label class="px-4 py-2 bg-accent text-on-accent rounded-lg hover:opacity-90 cursor-pointer text-sm">
      ...
      {media.length === 0 ? (
        <p class="text-ink-3">No media files.</p>
      ) : (
        <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
          {media.map((item: any) => (
            <div class="bg-surface border rounded-lg overflow-hidden group">
              {item.mimeType.startsWith('image/') ? (
                <img src={item.url} alt={item.altText || item.filename} class="w-full h-32 object-cover" />
              ) : (
                <div class="w-full h-32 flex items-center justify-center bg-surface-2 text-ink-3 text-sm">
                  {item.filename}
                </div>
              )}
              <div class="p-2">
                <p class="text-xs text-ink-2 truncate">{item.filename}</p>
                <p class="text-xs text-ink-3">{(item.size / 1024).toFixed(1)} KB</p>
                <div class="flex gap-1 mt-1">
                  <button onClick={() => copyUrl(item.url)} class="text-xs text-accent hover:underline">Copy URL</button>
